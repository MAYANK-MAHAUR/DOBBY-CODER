import { FireworksAIStream, FireworksAIStreamPayload } from "@/utils/FireworksAIStream";
...
- Please ONLY return the full ${framework} code starting with the imports, nothing else. It's very important for my job that you only return the ${framework} code with imports.
- DO NOT START WITH \`\`\`typescript or \`\`\`javascript or \`\`\`tsx or \`\`\`.
`;
  console.log(systemPrompt);
  const payload: FireworksAIStreamPayload = {
    model,
    messages: [
      {
        role: "system",
  ...
  const stream = await FireworksAIStream(payload);
